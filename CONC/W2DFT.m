...
function [W2DFT_original, W2DFT_con] = W2DFT(signal, est, NFFT_omega, NFFT_eta)
% inputs:
...
switch est
    case 1
        ...
        omega_shift = round(omega_bins.' -  NFFT_omega*imag(W2DFT_hRdhT ./ W2DFT_hRhT)/2/pi );
        eta_shift   = round(eta_bins -  NFFT_eta*imag(W2DFT_dhRhT ./ W2DFT_hRhT)/2/pi );
    case 2
        hT   = blackman_harris_window( T, 0, 0).';
        dhT  = blackman_harris_window( T, 1, 0).';
        ...
        W2DFT_hRd2hT      = fftshift(fftshift(fft2(signal .* hRd2hT,   NFFT_omega,  NFFT_eta),1),2);
        ...
        W2DFT_d2hRhT      = fftshift(fftshift(fft2(signal .* d2hRhT,   NFFT_omega,  NFFT_eta),1),2);

        omega_shift = round(omega_bins.' + NFFT_omega * imag((W2DFT_hRdthT .* W2DFT_hRdhT - W2DFT_hRd2hT .* W2DFT_hRthT)./(W2DFT_hRdhT .* W2DFT_hRthT - W2DFT_hRdthT .* W2DFT_hRhT)./2/pi));
        eta_shift = round(eta_bins   + NFFT_eta * imag((W2DFT_drhRhT .* W2DFT_dhRhT - W2DFT_d2hRhT .* W2DFT_rhRhT)./(W2DFT_dhRhT .* W2DFT_rhRhT - W2DFT_drhRhT .* W2DFT_hRhT)./2/pi));
    ...
    case 3
        ...
        W2DFT_hRthT       = fftshift(fftshift(fft2(signal .* hRthT,    NFFT_omega,  NFFT_eta),1),2);
        W2DFT_hRdthT      = fftshift(fftshift(fft2(signal .* hRdthT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_hRt2hT      = fftshift(fftshift(fft2(signal .* hRt2hT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_rhRhT       = fftshift(fftshift(fft2(signal .* rhRhT,    NFFT_omega,  NFFT_eta),1),2);
        W2DFT_drhRhT      = fftshift(fftshift(fft2(signal .* drhRhT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_r2hRhT      = fftshift(fftshift(fft2(signal .* r2hRhT,   NFFT_omega,  NFFT_eta),1),2);

        omega_shift   = round(omega_bins.' - NFFT_omega * imag((W2DFT_hRdthT .* W2DFT_hRthT - W2DFT_hRdhT.*W2DFT_hRt2hT + W2DFT_hRthT .* W2DFT_hRhT)./(W2DFT_hRthT.^2 - W2DFT_hRt2hT.*W2DFT_hRhT))./2/pi);
        eta_shift = round(eta_bins - NFFT_eta * imag((W2DFT_drhRhT .* W2DFT_rhRhT - W2DFT_dhRhT.*W2DFT_r2hRhT + W2DFT_rhRhT .* W2DFT_hRhT)./(W2DFT_rhRhT.^2 - W2DFT_r2hRhT.*W2DFT_hRhT))./2/pi);
end
...
for i = 1 : NFFT_omega
    for j = 1 : NFFT_eta
        omega_idx = omega_shift(i,j);
        eta_idx = eta_shift(i,j);
