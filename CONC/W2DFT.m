...
switch est
    case 1
        ...
        dwRwT          = dwR .* wT;
        wRdwT          = wR  .* dwT;
        W2DFT_wRwT     = fftshift(fftshift(fft2(signal .* wRwT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_wRdwT    = fftshift(fftshift(fft2(signal .* wRdwT,  NFFT_omega,  NFFT_eta),1),2);
        W2DFT_dwRwT    = fftshift(fftshift(fft2(signal .* dwRwT,  NFFT_omega,  NFFT_eta),1),2);
    ...
    case 2
        ...
        W2DFT_wRdwT       = fftshift(fftshift(fft2(signal .* wRdwT,    NFFT_omega,  NFFT_eta),1),2);
        W2DFT_dwRwT       = fftshift(fftshift(fft2(signal .* dwRwT,    NFFT_omega,  NFFT_eta),1),2);
        W2DFT_wRtwT       = fftshift(fftshift(fft2(signal .* wRtwT,    NFFT_omega,  NFFT_eta),1),2);
        W2DFT_wRdtwT      = fftshift(fftshift(fft2(signal .* wRdtwT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_wRd2wT      = fftshift(fftshift(fft2(signal .* wRd2wT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_rwRwT       = fftshift(fftshift(fft2(signal .* rwRwT,    NFFT_omega,  NFFT_eta),1),2);
        W2DFT_drwRwT      = fftshift(fftshift(fft2(signal .* drwRwT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_d2wRwT      = fftshift(fftshift(fft2(signal .* d2wRwT,   NFFT_omega,  NFFT_eta),1),2);
    ...
    case 3
        ...
        W2DFT_wRtwT       = fftshift(fftshift(fft2(signal .* wRtwT,    NFFT_omega,  NFFT_eta),1),2);
        W2DFT_wRdtwT      = fftshift(fftshift(fft2(signal .* wRdtwT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_wRt2wT      = fftshift(fftshift(fft2(signal .* wRt2wT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_rwRwT       = fftshift(fftshift(fft2(signal .* rwRwT,    NFFT_omega,  NFFT_eta),1),2);
        W2DFT_drwRwT      = fftshift(fftshift(fft2(signal .* drwRwT,   NFFT_omega,  NFFT_eta),1),2);
        W2DFT_r2wRwT      = fftshift(fftshift(fft2(signal .* r2wRwT,   NFFT_omega,  NFFT_eta),1),2);
...
for i = 1 : NFFT_omega
    for j = 1 : NFFT_eta
        ...
        W2DFT_con(omega_idx, eta_idx) = W2DFT_con(omega_idx, eta_idx) + abs(W2DFT_wRwT(i,j))^2;
    end
end
